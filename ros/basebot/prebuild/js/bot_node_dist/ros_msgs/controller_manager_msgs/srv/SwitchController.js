...
class SwitchControllerRequest {
  ...
  static getMessageSize(object) {
    let length = 0;
    for(let i = 0; i < object.start_controllers.length; ++i) {
      length += 4 + _getByteLength(object.start_controllers[i]);
    }
    for(let i = 0; i < object.stop_controllers.length; ++i) {
      length += 4 + _getByteLength(object.stop_controllers[i]);
    }
    // 21 is precalculated sum of the constant length fields
    return length + 21;
  }

  static datatype() {
    // Returns string type for a controller_manager_msgs/SwitchControllerRequest object
...
class SwitchControllerResponse {
  ...
  static datatype() {
    // Returns string type for a controller_manager_msgs/SwitchControllerResponse object
