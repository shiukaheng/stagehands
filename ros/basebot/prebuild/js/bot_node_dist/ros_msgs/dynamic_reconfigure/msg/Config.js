...
const BoolParameter = require('./BoolParameter.js');
const IntParameter = require('./IntParameter.js');
const StrParameter = require('./StrParameter.js');
const DoubleParameter = require('./DoubleParameter.js');
const GroupState = require('./GroupState.js');
...
class Config {
  ...
  static getMessageSize(object) {
    let length = 0;
    for(let i = 0; i < object.bools.length; ++i) {
      length += BoolParameter.getMessageSize(object.bools[i]);
    }
    for(let i = 0; i < object.ints.length; ++i) {
      length += IntParameter.getMessageSize(object.ints[i]);
    }
    for(let i = 0; i < object.strs.length; ++i) {
      length += StrParameter.getMessageSize(object.strs[i]);
    }
    for(let i = 0; i < object.doubles.length; ++i) {
      length += DoubleParameter.getMessageSize(object.doubles[i]);
    }
    for(let i = 0; i < object.groups.length; ++i) {
      length += GroupState.getMessageSize(object.groups[i]);
    }
    // 20 is precalculated sum of the constant length fields
    return length + 20;
  }

  static datatype() {
    // Returns string type for a dynamic_reconfigure/Config object
