...
class GetModelPropertiesRequest {
  ...
  static datatype() {
    // Returns string type for a gazebo_msgs/GetModelPropertiesRequest object
...
class GetModelPropertiesResponse {
  ...
  static getMessageSize(object) {
    let length = 0;
    length += _getByteLength(object.parent_model_name);
    length += _getByteLength(object.canonical_body_name);
    for(let i = 0; i < object.body_names.length; ++i) {
      length += 4 + _getByteLength(object.body_names[i]);
    }
    for(let i = 0; i < object.geom_names.length; ++i) {
      length += 4 + _getByteLength(object.geom_names[i]);
    }
    for(let i = 0; i < object.joint_names.length; ++i) {
      length += 4 + _getByteLength(object.joint_names[i]);
    }
    for(let i = 0; i < object.child_model_names.length; ++i) {
      length += 4 + _getByteLength(object.child_model_names[i]);
    }
    length += _getByteLength(object.status_message);
    // 30 is precalculated sum of the constant length fields
    return length + 30;
  }

  static datatype() {
    // Returns string type for a gazebo_msgs/GetModelPropertiesResponse object
