...
const LaserEcho = require('./LaserEcho.js');
...
class MultiEchoLaserScan {
  ...
  static getMessageSize(object) {
    ...
    for(let i = 0; i < object.ranges.length; ++i) {
      length += LaserEcho.getMessageSize(object.ranges[i]);
    }
    for(let i = 0; i < object.intensities.length; ++i) {
      length += LaserEcho.getMessageSize(object.intensities[i]);
    }
    // 36 is precalculated sum of the constant length fields
    return length + 36;
  }

  static datatype() {
    // Returns string type for a sensor_msgs/MultiEchoLaserScan object
